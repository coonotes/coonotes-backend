...
describe("User", () => {
    ...
    describe("username", () => {
        ...
        it("should rename the username", () => {
            const whenRenamed = (cb) => (user) => cb(user.rename("newName"));
            const thenItShouldReturnRenamedUser = (cb) => expect(cb).to.have.property("username", "newName");

            givenAUser(
                whenRenamed(
                    thenItShouldReturnRenamedUser
                )
            );
        });
    });
